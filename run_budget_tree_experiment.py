...
def write_csv(path: Path, rows: List[Dict[str, object]]) -> None:
    """Write rows with a stable schema even if methods expose different aux fields.

    Some methods produce method-specific diagnostics, e.g. BFS/greedy expose
    `generated_candidates` while chain does not. Using only `rows[0].keys()`
    can therefore fail when later rows contain extra keys.
    """
    if not rows:
    ...
    # Keep preferred metrics first, then append all method-specific diagnostics.
    all_keys = []
    seen = set()
